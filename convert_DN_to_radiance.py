...
def create_radiance_images(rootname, outputname):
	...
	output_image = drv.Create(outputname, band1.RasterXSize, band1.RasterYSize, 6, gdalconst.GDT_Float32)
	...
	for new_index, file_index in zip([1, 2, 3, 4, 5, 6], [1, 2, 3, 4, 5, 7]):
		...
		logging.debug("Band %d. Mult = %f. Add = %f", file_index, radiance_mult[new_index-1], radiance_add[new_index-1])
		new_band.WriteArray( (radiance_mult[new_index-1] * values) + radiance_add[new_index-1])
